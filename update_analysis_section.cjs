...
code = code.replace(/!mainFeaturedAnalysis/g, '!mainAnalysis');
code = code.replace(/const dateDisplay = mainFeaturedAnalysis\.publishedAt \|\| mainFeaturedAnalysis\.date;/, 'const dateDisplay = mainAnalysis?.publishedAt || (mainAnalysis as any)?.date;');
