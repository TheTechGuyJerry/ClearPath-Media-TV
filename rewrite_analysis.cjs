...
export const AnalysisAndFeaturesSection: React.FC<AnalysisAndFeaturesSectionProps> = ({
  ...
  const authorDisplay = mainAnalysis?.authorName || 'ClearPath';
        ...
        <div className="lg:col-span-8 flex flex-col justify-between h-full space-y-6">
          <div className="space-y-4">
            ...
            <p className="text-sm sm:text-base text-slate-600 dark:text-on-surface-variant leading-relaxed max-w-3xl line-clamp-3">
              {mainAnalysis?.excerpt || 'Articles for this section will appear here once they are published from the CMS.'}
            </p>

            <div className="flex items-center gap-3 pt-2">
              <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-surface-container flex items-center justify-center text-[10px] font-bold text-slate-700 dark:text-on-surface shadow-xs">
                {authorDisplay[0]}
              </div>
              <span className="text-[11px] font-bold text-slate-800 dark:text-on-surface uppercase tracking-wide">
                {authorDisplay}
              </span>
            </div>
            ...
            <div className="w-full aspect-[21/9] sm:aspect-[16/6] md:aspect-[21/9] rounded-2xl overflow-hidden shadow-xs mt-4 group">
              <Link to={mainAnalysis ? getArticleUrl(mainAnalysis as any, 'todays-brief') : '#'}>
                <img
                  src={mainAnalysis?.coverImage || 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&q=80&w=1200'}
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-outline-variant/40 mt-auto">
            <Link
              to={mainAnalysis ? getArticleUrl(mainAnalysis as any, 'todays-brief') : '#'}
