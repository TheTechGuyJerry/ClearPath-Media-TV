...
import { Tag, Search, ArrowRight, User } from 'lucide-react';
import SEO from '../components/SEO';
import { ALL_TOPICS, SAMPLE_DAILY_ARTICLES, DailyArticle } from '../data/clearpath_daily_data';
...
export default function TopicPage() {
  ...
  const currentTopic = ALL_TOPICS.find(t => t.id === slug) || {
    id: slug || 'all',
    name: slug ? slug.replace(/-/g, ' ').toUpperCase() : 'Topic Archive'
  };

  const filteredArticles = SAMPLE_DAILY_ARTICLES.filter(a => {
    const hasTag = slug ? a.topicTags.includes(slug) : true;
  ...
  return (
    <div className="w-full min-h-screen bg-background font-sans">
      <SEO
        title={`Topic: ${currentTopic.name} — ClearPath Daily`}
        description={`Explore all ClearPath Daily analysis, intelligence briefs, and publications regarding ${currentTopic.name}.`}
      />

      {/* Header Banner */}
      <div className="bg-slate-950 text-white border-b border-outline-variant py-10 md:py-12 px-margin-mobile md:px-margin-desktop">
        <div className="max-w-container-max mx-auto">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold font-serif text-white mb-2">
            #{currentTopic.name}
          </h1>
          <p className="text-sm text-slate-300">
            Curated analysis and intelligence publications focused on {currentTopic.name}.
          </p>
        </div>
      </div>
      ...
      <main className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-10 space-y-8">
        ...
        <div className="p-4 bg-surface-bright border border-outline-variant rounded-xl flex items-center gap-3">
          ...
          <input
            type="text"
            placeholder={`Search articles in #${currentTopic.name}...`}
            value={searchQuery}
        ...
        {filteredArticles.length === 0 ? (
          <div className="p-12 text-center bg-surface-bright border border-outline-variant rounded-2xl">
            <Tag className="w-12 h-12 text-on-surface-variant mx-auto mb-3 opacity-60" />
            <h3 className="text-lg font-bold text-on-surface mb-1">
              No articles tagged with #{currentTopic.name}
            </h3>
            <p className="text-xs text-on-surface-variant mb-6">
              Check back soon for new analysis tagged with this topic.
            </p>
            <Link
              to="/clearpath-daily"
              className="px-5 py-2.5 bg-primary text-on-primary text-xs font-bold uppercase tracking-wider rounded-lg shadow-sm"
            >
              Return to ClearPath Daily
            </Link>
          </div>
        ) : (
